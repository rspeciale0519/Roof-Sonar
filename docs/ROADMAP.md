...
- [x] Footprint-based roofing squares (migration 0016): measure roof plan area from FEMA USA Structures building footprints — point-in-polygon match, nearest-within-15m fallback — instead of living-area×slope, which ran 20-33% low vs Planimeter. scripts/apply-footprint-squares.ts tile-parallel-downloads footprints per county; modal shows "· aerial" when footprint-sourced. Applied to all 9 counties (2026-06-12): ~1.92M properties measured. Coverage Pinellas/Pasco/Orange 90-91%, Volusia/Seminole/Hillsborough 82-87%, Sumter 74%, Lake 69%, Marion 51%. 5-home spot-check 4/5 within ±11% of Planimeter; no regression (unmatched keep living-area).
  - [ ] Rural-county refinement: Sumter/Lake/Marion have many single-family homes whose parcel-centroid geocode sits >15m from the roof on large lots (Marion: 57,852 single-fam on fallback). Improve the nearest-footprint fallback with a larger radius bounded by living-area size (reject barns/sheds) so it reaches the house without grabbing outbuildings; re-run those 3 counties.
