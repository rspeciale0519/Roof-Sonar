...
### Sumter
- **Parcels (free, has lat/lng)**: `https://services8.arcgis.com/FTrtUCmxaVKdPC5e/arcgis/rest/services/Parcels_gdb/FeatureServer/0` — PIN, LATITUDE/LONGITUDE, Site_Addr_/Site_Str/Site_City/Site_Zip, Owners_Nam, Mailing_*, DOR_LUC (numeric), AYB/EYB, LivingArea (Query+Extract enabled). Address points: `Address_Public/FeatureServer/0`.
- **Official bulk channel (found 2026-06-10, browser check)**: qPublic "Data Download" tab (`Application.aspx?AppID=1207&LayerID=36374&PageTypeID=16&PageID=15321`) publishes **public read-only SFTP credentials**: `sftp.sumterpa.com:22`, user `scpadownload` (password posted on that page). Contents not yet listed (pending Rob's OK to connect) — Sumter's CAMA is Vision; Vision extracts sometimes include a permit table. CHECK THIS FIRST.
- **Permits**: NO permits on qPublic parcel pages — verified in-browser on a real Villages home (1093 Reidville Rd, D26E013: Summary/Owner/Land/Building/Sales/Valuation only; Building Data does include **Roof Cover + Actual/Effective Year Built** — proxy fields). PA GIS viewer also has none. County Building Services = Citizenserve installation 445 — its portal exposes "Permits Issued by Jurisdiction"/"Issued Permits" reports (need browser session) proving the export is one query away. **Records request via NextRequest `https://sumtercountyfl.nextrequest.com/`** (ask for legacy pre-Citizenserve too). **Wildwood**: CivicGov portal `civicgov4.com/fl_wildwood`; request via JustFOIA `wildwoodfl.justfoia.com` / dsdinfo@wildwood-fl.gov.
- PA records custodian: christine.benitez@sumtercountyfl.gov; GIS: GISHelpdesk@SumterCountyFL.gov.
...
- **PA parcel pages DO show permits** (verified in-browser 2026-06-10): `PRC.aspx?key=<ALT_KEY>&YR=2026` has a "Planning and Building — Permit Search" section (Permit Number / Date Issued / Date Completed / Description). Per-parcel cross-check only; CDPlus remains the bulk path. Search URL pattern: `PropertySearch.aspx?SearchBy=Address&Parms=<address>`.
